...
namespace nsBTree
{
	...
	{
		static struct Tree * p;
		struct Tree * retNode;

		if (nullptr == node) {
			retNode = newTreeNode(data);
			retNode->parent = p;///WTF?
	...
	{
		if (nullptr == node || (nullptr == node->left && nullptr == node->right))
			return 0;
	...
	//A node's successor is the one with the next largest value in the tree..
	//If all keys are distinct, the successor of a nodeX is the node with
	//-	the smallest key greater than nodeX.key
	struct Tree * succesorInOrder(struct Tree * node)
	{
		//if the node has right child, successor is the Tree-Minimum
		if (nullptr != node->right)
			return minTree(node->right);

		//the node without right child, and search back up the tree
		//until we find the first-right-hand-turn;
		struct Tree * y = node->parent;
		while (nullptr != y && node == y->right) {
			node = y;
			y = y->parent;
		}
	...
	struct Tree * predecessorInOrder(struct Tree * node)
	{
	...
	///
	struct Tree * lowestCommonAncestor(struct Tree * node, struct Tree * p, struct Tree * q)
	{
	...
	/************************************************************************
	  In breadth-first-search (BFS) we start with the root,
	  move left-to-right across the second level,then move
	  left-to-right across the third level, and so on.

	  We continue the search until either we have examined
	  all of the nodes or we find the node we are searching
	  for. 

	  THE TIME to find a node is O(n), so this type of search
	  is best avoided for large trees. A BFS also uses a large
	  amount of memory because it is necessary to track the
	  child nodes for all nodes on a given level while searching
	  that level...
	/************************************************************************/
